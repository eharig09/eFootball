...
LEFT, RIGHT, TOP, BOTTOM = 58, 18, 22, 38
...
def series(rows: Iterable[dict[str, Any]], key: str, label: str, *,
    ...
    for index, item in enumerate(values):
        ...
        item["overlay_y"] = TOP + PLOT_H * (raw_high - item["value"]) / (raw_high - raw_low or 1)
    ...
    return {"key": key, "label": label, "format": value_format, "path": _path(values),
            "overlay_path": _path(values, "overlay_y"), "values": values, "low": raw_low,
