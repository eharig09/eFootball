...
from sports_aggregator.cfb.margin_common_sample import report as margin_common_sample_report
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--drives-train-from-year", type=int, default=2022,
                   help="Earliest season allowed into each prior-season xdrives fold.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    first, last = _years(args, required=args.command in {"prepare", "coverage", "coverage-audit", "backfill-readiness", "run", "score-calibration", "margin-feature-ablation", "margin-common-sample"})
    ...
    elif args.command == "margin-common-sample":
        payload = margin_common_sample_report(
            repository,
            from_season=int(first),
            to_season=int(last),
            training_from_season=args.points_train_from_year,
